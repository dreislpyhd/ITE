...
// Get all community concerns with user information (filtered by encoder's assigned streets)
try {
    ...
    if (in_array($user_role, ['encoder1', 'encoder2', 'encoder3'])) {
        ...
        $stmt = $conn->prepare("
            SELECT cc.*, u.full_name, u.email, u.street
    ...
    $concerns = [];
    error_log("Community concerns query error: " . $e->getMessage());
}
