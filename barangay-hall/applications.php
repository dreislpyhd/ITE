        ...
        switch ($_POST['action']) {
                ...
                try {
                    ...
                    if ($stmt->execute([$new_status, $application_id])) {
                        ...
                        if ($emailService && strtolower($new_status) === 'approved' && stripos($remarks, 'ready for pick-up') !== false) {
                            $infoStmt = $conn->prepare("SELECT a.id, a.user_id, a.application_date, a.service_type,
                                u.full_name, u.email,
                            ...
                            if ($row = $infoStmt->fetch()) {
                                $year = !empty($row['application_date']) ? date('Y', strtotime($row['application_date'])) : date('Y');
            ...
            case 'generate_certificate':
                ...
                try {
                    ...
                    $stmt = $conn->prepare("UPDATE applications SET status = 'processing' WHERE id = ?");
...
// Get all applications (filtered by encoder's assigned streets)
try {
    $stmt = $conn->prepare("
        SELECT 
            a.id,
    ...
    // Filter applications based on encoder role
    $user_role = $_SESSION['role'];
    if (in_array($user_role, ['encoder1', 'encoder2', 'encoder3'])) {
        $assigned_streets = getStreetsForEncoder($user_role);
        $applications = array_filter($all_applications, function($app) use ($assigned_streets) {
            return in_array($app['street'], $assigned_streets);
        });
    } else {
        // barangay_staff and barangay_hall can see all applications
...
try {
    $stmt = $conn->prepare("UPDATE application_notifications SET is_read = 1 WHERE user_id = ? AND is_read = 0");
...
// Get status counts for statistics (filtered by encoder's assigned streets)
try {
    $user_role = $_SESSION['role'];
    ...
    if (in_array($user_role, ['encoder1', 'encoder2', 'encoder3'])) {
        ...
        $stmt = $conn->prepare("
            ...
            FROM applications a
            LEFT JOIN users u ON a.user_id = u.id
            WHERE u.street IN ($street_placeholders)
            GROUP BY a.status
        ");
...
<body class="font-poppins bg-gray-50">
                                 ...
                                 <?php foreach ($applications as $app): ?>
                                     <tr class="hover:bg-gray-50">
                                         <td class="px-6 py-4">
                                             ...
                                             <div class="text-xs text-gray-500"><?php echo $app['application_date'] ? date('Y', strtotime($app['application_date'])) : date('Y'); ?></div>
                                         </td>
                                         <td class="px-6 py-4">
                                             <div>
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="text-sm text-gray-900">
                                                <?php echo $app['application_date'] ? date('M d, Y', strtotime($app['application_date'])) : 'N/A'; ?>
                                            </div>
                                            <div class="text-xs text-gray-500">
                                                <?php echo $app['application_date'] ? date('g:i A', strtotime($app['application_date'])) : ''; ?>
                                            </div>
    ...
    <script>
        ...
        function downloadCertificate() {
            ...
            // Get the certificate content from the modal
            const certificateContentEl = document.getElementById('certificateContent');
            if (!certificateContentEl || !certificateContentEl.innerHTML || certificateContentEl.innerHTML.includes('Certificate content will be loaded here')) {
                alert('Please view the certificate first before downloading.');
                return;
            }
            
            // Use html2pdf to generate and download PDF from current page content
            const element = certificateContentEl;
            const opt = {
                margin: [10, 10, 10, 10],
                filename: 'Certificate_' + currentApplication.id + '.pdf',
