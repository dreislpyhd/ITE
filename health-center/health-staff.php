    ...
    try {
        if ($action === 'approve') {
            ...
            try {
                ...
                $log_stmt->execute([
                    'patient_approved',
                    'Approved patient registration for ' . $registration['full_name'],
            ...
            try {
                ...
                ");
                // Get patient name
                $name_stmt = $conn->prepare("SELECT u.full_name FROM patient_registrations pr JOIN users u ON pr.user_id = u.id WHERE pr.id = ?");
                $name_stmt->execute([$registration_id]);
                $patient_name = $name_stmt->fetchColumn();
                
                $log_stmt->execute([
                    'patient_rejected',
                    'Rejected patient registration for ' . $patient_name,
                    'patient_registration',
                    $registration_id,
                    $patient_name,
                    $user_id,
            ...
            // Try to move to deleted_patients table (archive)
            try {
                ...
                $stmt = $conn->prepare("SELECT * FROM patient_registrations WHERE id = ?");
                $stmt->execute([$registration_id]);
                $patient = $stmt->fetch();
                
                if ($patient) {
                    // Try to insert into deleted_patients
                    try {
                        $archive_stmt = $conn->prepare("
                            INSERT INTO deleted_patients (user_id, blood_type, emergency_contact, medical_history, current_medications, insurance_info, status, created_at, deleted_at, deleted_by)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), ?)
                        ");
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <?php echo date('M j, Y', strtotime($registration['created_at'])); ?>
                                             ...
                                             <div class="flex space-x-2">
                                                 ...
                                                 <form method="POST" class="inline" onsubmit="return confirm('Are you sure you want to archive this patient registration? You can restore it from Archives.');">
                                                     <input type="hidden" name="registration_id" value="<?php echo $registration['id']; ?>">
                                                     <input type="hidden" name="action" value="delete_patient">
                                                     <button type="submit" class="bg-orange-500 hover:bg-orange-600 text-white px-3 py-1 rounded-lg text-sm font-medium transition duration-300">
                                                         Archive
                                                     </button>
                                                 </form>
                                             </div>
