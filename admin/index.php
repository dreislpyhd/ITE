...
$stats = [];
try {
    // Total users (only count encoders, residents, and health staff)
    $stmt = $conn->query("SELECT COUNT(*) as total FROM users WHERE role IN ('encoder1', 'encoder2', 'encoder3', 'resident', 'health_staff')");
    ...
    $stats['encoders'] = ($role_counts['encoder1'] ?? 0) + ($role_counts['encoder2'] ?? 0) + ($role_counts['encoder3'] ?? 0);
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6 lg:mb-8">
                ...
                <div class="bg-white rounded-xl shadow-md p-4 lg:p-6 border-l-4 border-orange-500">
                    <div class="flex items-center">
                        ...
                        <div class="ml-3 lg:ml-4">
                            <p class="text-xs lg:text-sm font-medium text-gray-600">Encoders</p>
                            <p class="text-xl lg:text-2xl font-semibold text-gray-900"><?php echo $stats['encoders']; ?></p>
                        </div>
                    </div>
                </div>
