        ...
        switch ($_POST['action']) {
                            ...
                            try {
                                ...
                                foreach ($tables_to_clean as $table) {
                                    try {
                                        ...
                                        error_log("Deleted from $table: " . $stmt->rowCount() . " rows");
                                    } catch (Exception $e) {
                                ...
                                error_log("User delete attempt: user_id=$user_id, result=$result, rowCount=$deleted_count");
                                ...
                                if ($result && $deleted_count > 0) {
                                    ...
                                    header('Location: users.php?message=User+deleted+successfully&deleted=' . $user_id . '&t=' . time());
                                    exit();
                                } else {
                                    $conn->rollBack();
                                    $conn->exec("PRAGMA foreign_keys = ON");
                                    $error = 'Failed to delete user. User may not exist.';
                                    error_log("Delete failed: user_id=$user_id, rowCount=$deleted_count");
                                }
                            } catch (Exception $e) {
                                $conn->rollBack();
                                $conn->exec("PRAGMA foreign_keys = ON");
            ...
            case 'create_user':
                if (isset($_POST['full_name']) && isset($_POST['email']) && isset($_POST['role'])) {
                    ...
                    if (empty($full_name) || empty($email) || empty($role)) {
                    ...
                    } elseif (!in_array($role, ['encoder1', 'encoder2', 'encoder3', 'health_center'])) {
                        ...
                        try {
                            ...
                            if ($check_stmt->fetch()) {
                                ...
                                // Check if encoder role already exists (only one account per encoder role)
                                if (in_array($role, ['encoder1', 'encoder2', 'encoder3'])) {
                                    $role_check_stmt = $conn->prepare("SELECT id FROM users WHERE role = ?");
                                    $role_check_stmt->execute([$role]);
                                    if ($role_check_stmt->fetch()) {
                                        $role_names = [
                                            'encoder1' => 'Encoder 1',
                                            'encoder2' => 'Encoder 2',
                                            'encoder3' => 'Encoder 3'
                                        ];
                                        $error = $role_names[$role] . ' account already exists. Only one account is allowed per encoder role.';
                                        break;
                                    }
                                }
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6">
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-semibold text-gray-900">
                                <?php 
                                try {
                                    $total_stmt = $conn->query("SELECT COUNT(*) FROM users WHERE role IN ('encoder1', 'encoder2', 'encoder3', 'resident', 'health_staff')");
                ...
                <div class="bg-white rounded-xl shadow-md p-4 lg:p-6 border-l-4 border-green-500">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <p class="text-sm font-medium text-gray-600">Encoders</p>
                            <p class="text-2xl font-semibold text-gray-900">
                                <?php 
                                try {
                                    $encoder_stmt = $conn->query("SELECT COUNT(*) FROM users WHERE role IN ('encoder1', 'encoder2', 'encoder3')");
                                    echo $encoder_stmt->fetchColumn();
                                } catch (Exception $e) {
                                    echo '0';
                                }
                                ?>
                ...
                <div class="bg-white rounded-xl shadow-md p-4 lg:p-6 border-l-4 border-purple-500">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <p class="text-sm font-medium text-gray-600">Health Staff</p>
                            <p class="text-2xl font-semibold text-gray-900">
                                <?php 
                                try {
                                    $health_staff_stmt = $conn->query("SELECT COUNT(*) FROM users WHERE role = 'health_staff'");
                                    echo $health_staff_stmt->fetchColumn();
                                } catch (Exception $e) {
                                    echo '0';
                                }
                                ?>
            ...
            <div class="bg-white rounded-xl shadow-md p-6 mb-6">
                <form method="GET" class="flex flex-wrap gap-4 items-end">
                    ...
                    <div>
                        ...
                        <select id="role" name="role" onchange="this.form.submit()" class="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-barangay-orange focus:border-transparent">
                            <option value="">All Roles</option>
                            <option value="resident" <?php echo $role_filter === 'resident' ? 'selected' : ''; ?>>Resident</option>
                            <option value="encoder1" <?php echo $role_filter === 'encoder1' ? 'selected' : ''; ?>>Encoder 1</option>
                            <option value="encoder2" <?php echo $role_filter === 'encoder2' ? 'selected' : ''; ?>>Encoder 2</option>
                            <option value="encoder3" <?php echo $role_filter === 'encoder3' ? 'selected' : ''; ?>>Encoder 3</option>
                            <option value="health_center" <?php echo $role_filter === 'health_center' ? 'selected' : ''; ?>>Health Center Admin</option>
            ...
            <form method="POST" id="roleForm">
                ...
                <div class="mb-4">
                    ...
                    <select id="newRole" name="new_role" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-barangay-orange focus:border-transparent">
                        ...
                        <option value="encoder1">Encoder 1</option>
                        <option value="encoder2">Encoder 2</option>
                        <option value="encoder3">Encoder 3</option>
